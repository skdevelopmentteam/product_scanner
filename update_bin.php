...
if($scanned_barcode == '' || $barcode_length == 0){
    ...
    if($isBinOrSlab === false){
        ...
        if(count($_SESSION['scanned_slabs']) > 0){
            $bin_details = mysqli_fetch_assoc(mysqli_query($link,"SELECT `id`,`name` FROM `bins` WHERE name='$bin_name'"));
			if($bin_details['name'] != 'N/A'){
				$count = 0;
				...
				echo '<div class="text-danger">Bin Not Selected</div> <br />';
			}
            ?>
